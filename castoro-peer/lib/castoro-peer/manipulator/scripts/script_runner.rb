...
module Castoro::Peer::Manipulator #:nodoc:
  ...
  class ScriptRunner
    ...
    def self.start options
      ...
      user = config["user"] || Service::DEFAULT_SETTINGS["user"]
...
#      raise "Don't run as root user." if uid == 0
      ...
      if options[:daemon]
        ...
        logger = if config["logger"]
                   ...
                   eval(Service::DEFAULT_SETTINGS["logger"]).call(options[:log])
                 end
        ...
        fork {
          ...
          fork {
            ...
            begin
              manipulator = Service.new(config, logger)
        ...
        manipulator = Service.new(config, Logger.new(STDOUT))
    ...
    def self.setup options
      ...
      open(options[:conf], "w") { |f|
        f.puts Service::SETTING_TEMPLATE
      }
